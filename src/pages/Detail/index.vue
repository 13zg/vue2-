    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :imageList1="skuImageList" />
          <!-- 小图列表 -->
          <ImageList :imageList2="skuImageList" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="choose1 in spuSaleAttrList" :key="choose1.id">
                <dt class="title">选择{{ choose1.saleAttrName }}</dt>
                <dd
                  :class="{ active: choose2.isChecked == 1 }"
                  v-for="choose2 in choose1.spuSaleAttrValueList"
                  :key="choose2.id"
                  @click="changeBgc(choose2, choose1.spuSaleAttrValueList)"
                >
                  {{ choose2.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 不用编程式路由进行跳转，因为在跳转之前要先将数据发起请求存储到服务器 -->
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      /* 当数据未回来时，skuInfo为空，此时skuImageList会为undefined，
      所以取为空数组,而在子组件中会对其进行url读值，所以还要包含一个空对象 */
      return this.skuInfo.skuImageList || [{}];
    },
  },
  mounted() {
    // 派发请求，获取参数
    this.$store.dispatch("detail/getDetail", this.$route.params.goodid);
  },
  methods: {
    //实现点击选项切换高亮属性(排他操作)
    changeBgc(target, classify) {
      classify.forEach((val) => {
        val.isChecked = 0;
      });
      target.isChecked = 1;
    },
    ...
    changeNum(e) {
      ...
      /* 
      1、如果输入值小于1，则默认为1
      2、如果是字符串(不是纯数字)，则和1相乘，判断是否是NaN，如果是，也默认为1
      */
    ...
    async addCart() {
      // 派发获取请求
      try {
        await this.$store.dispatch("detail/getCart", {
