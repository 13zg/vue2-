<template>
  <div class="spec-preview">
    <img :src="imageList1[index].imgUrl" />
    <!-- event专门用于事件触发 -->
    <div class="event" @mousemove="handler"></div>
    <div class="big">
      <img :src="imageList1[index].imgUrl" ref="big" />
    </div>
    <div class="mask" ref="mask"></div>
  </div>
</template>
<script>
export default {
  name: "Zoom",
  props: ["imageList1"],
  data() {
    return {
      //默认显示索引为0的图片
      index: 0,
    };
  },
  methods: {
    changeImage(val) {
      // 将轮播图变化的currentIndex的值赋值给index，实现同步
      this.index = val;
    },
    //实现放大镜效果
    handler(e) {
      let mask = this.$refs.mask;
      let big = this.$refs.big;
      //实现mask跟随鼠标移动
      //  通过改变定位的top、left实现
      let x = e.offsetX - mask.offsetWidth / 2;
      let y = e.offsetY - mask.offsetHeight / 2;
      if (x <= 0) {
        x = 0;
        /*  由于是正方形，且mask的宽高为preview的一半，
        所以两者相减的值就等于
        this.$refs.mask.offsetWidth或this.$refs.mask.offsetHeight  */
      } else if (x >= mask.offsetWidth) {
        x = mask.offsetWidth;
      }
      if (y <= 0) {
        y = 0;
      } else if (y >= mask.offsetHeight) {
        y = mask.offsetHeight;
      }
      mask.style.left = x + "px";
      mask.style.top = y + "px";
      //实现大图显示放大效果
      // 移动方向相反且距离为两倍
      big.style.left = -2 * x + "px";
      big.style.top = -2 * y + "px";
    },
  },
  mounted() {
    // 绑定全局事件总线，获取变化的index
    this.$bus.$on("changeImage", this.changeImage);
  },
};
</script>

<style lang="less">
.spec-preview {
  position: relative;
  width: 400px;
  height: 400px;
  border: 1px solid #ccc;

  img {
    width: 100%;
    height: 100%;
  }

  .event {
    width: 100%;
    height: 100%;
    position: absolute;
    top: 0;
    left: 0;
    z-index: 998;
  }

  .mask {
    width: 50%;
    height: 50%;
    background-color: rgba(0, 255, 0, 0.3);
    position: absolute;
    left: 0;
    top: 0;
    display: none;
  }

  .big {
    width: 100%;
    height: 100%;
    position: absolute;
    top: -1px;
    left: 100%;
    border: 1px solid #aaa;
    overflow: hidden;
    z-index: 998;
    display: none;
    background: white;

    img {
      width: 200%;
      max-width: 200%;
      height: 200%;
      position: absolute;
      left: 0;
      top: 0;
    }
  }

  .event:hover ~ .mask,
  .event:hover ~ .big {
    display: block;
  }
}
</style>
